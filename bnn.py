...
class BNN(nn.Module):
    ...
    def _log_prob_normal(self, input, mu=0., sigma=1.):
        """Log Prob maximised when mu(prediction) is close to input(target)
        We want High probability of input(target) under the distribution 
        parametrized by mu and sigma. 
        This will shift the mu closer to the target regime. 
        """
